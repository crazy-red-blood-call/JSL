...
import china from '@/assets/echarts/map/china.json'
import '@/assets/echarts/map/china.js'
import '@/assets/echarts/map/chinaMapOutline.js'

export const chineseMap = function (doms: any) {
    // const pie1 = ref(); // 创建DOM引用
    var myChart: any = echarts.init(doms.value)
    let options = ccc();
...
const fromLL = [114.4648, 31.2891];

const ccc = function () {
    echarts.registerMap('LKX', china as any);
    ...
    tempList.map(function (item: any[], i) {
        series.push(
            {
                type: 'lines',
                zlevel: 2,
